          ...
          inputSchema: {
            ...
            properties: {
              ...
              start_line: {
                type: "number",
                description:
                  "Start line number. Used to identify where to replace when the old_str matches multiple text chunks in the file.",
              },
            },
        ...
        switch (request.params.name) {
          case "view":
            if (
              ...
              !isViewArgs(request.params.arguments)
            ) {
          ...
          case "create":
            if (
              !request.params.arguments ||
              !isCreateArgs(request.params.arguments)
            ) {
          ...
          case "string_replace":
            if (
              !request.params.arguments ||
              !isStrReplaceArgs(request.params.arguments)
            ) {
              throw new ToolError(
                "Invalid arguments for string_replace command",
              ); // Fixed
            }
            if (request.params.arguments.old_str === "") {
              throw new ToolError(
                "old_str cannot be an empty string. That's too generic and will result in replacing a random empty string in the file.",
              );
            }
          ...
          case "insert":
            if (
              !request.params.arguments ||
              !isInsertArgs(request.params.arguments)
            ) {
          ...
          case "undo_edit":
            if (
              !request.params.arguments ||
              !isUndoEditArgs(request.params.arguments)
            ) {
